...
export default function Agents() {
  ...
  const { data, isLoading } = useQuery<{ data: Agent[]; total: number }>({
    queryKey: ["/api/agents", `?limit=${PAGE_SIZE}&offset=${page * PAGE_SIZE}`],
  });
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 py-6 space-y-4">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div>
          <h1 className="text-lg font-semibold" data-testid="text-agents-title">All Agents</h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            A total of {total.toLocaleString()} agent(s) found. Trust indicators show BAP-578 compliance level.
          </p>
        </div>
      </div>
              ...
              {isLoading ? (
                ...
                agents.map((agent) => (
                  <tr
                    ...
                    data-testid={`row-agent-${agent.address.slice(0, 8)}`}
                  >
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between gap-4 px-4 py-3 border-t">
            ...
            <div className="flex items-center gap-1">
              <Button size="sm" variant="outline" disabled={page === 0} onClick={() => setPage(page - 1)} data-testid="button-agents-prev">
                <ChevronLeft className="w-4 h-4" />
              </Button>
              <Button size="sm" variant="outline" disabled={page >= totalPages - 1} onClick={() => setPage(page + 1)} data-testid="button-agents-next">
